...
## 2026-09-19, UI-2 step 7: tests and the visual comparison

**Test coverage, against the prompt's list.** Nothing new had to be built for most of it, because each item was tested with the screen it belongs to. Where each lives:
- A status_only item shows no requiredBy, category or issue text: `roadmap.test.tsx` ("what a status-only viewer sees": the row's text is exactly the name and status). A **hidden item is absent**: added this step (`roadmap.test.tsx`, "hidden documents"): the API omits hidden items, so the test asserts the roadmap shows only what it is sent, and names, counts or groups nothing else. (The rule that hides an item is the backend's and is tested there.)
- A null category renders under "Uncategorised": `roadmap.test.tsx`. Status_only items sit under "Limited access", never under Uncategorised.
- The action queue's filter chips change the items and the count: `dashboard.test.tsx`. Upload buttons are disabled: `dashboard.test.tsx`, `roadmap.test.tsx`.
- The theme toggle adds and removes `dark` and persists: `theme.test.tsx`.
- The map's OpenStreetMap and CARTO credit, the "Sample positions" flag, and the tile URL swapping with the theme, with Leaflet's tile layer replaced by a recorder so no test touches the network: `map.test.tsx`.
- The superadmin route is inaccessible to a non-superadmin: `shell.test.tsx` and `admin.test.tsx`. The dev user switcher is absent in a production build, checked against a real production `vite build`, with a development build as a positive control: `build.test.ts`.
- The grep-style guard for "Document name TBC", "wallet", "VERI token" and the em dash across everything in `web/src` (plus AI-extracted, Guardian, forensic, ledger, passport and IOTA strings, hard-coded colours, and anything under 14px): `hygiene.test.ts`. It has positive controls, so it cannot pass by scanning nothing.
- **`vite build` is part of the suite** (`build.test.ts`, in a child process with `NODE_ENV=production`).
- Test-only changes this step: the map tests now load the lazy map chunk once in a `beforeAll` with a 90 second allowance, because in one full run on a busy machine the first map test spent more than its 8 second wait on the cold chunk load (it passed in the audit's run and in every rerun). No product code changed.

**Visual comparison, in a real browser, in both themes.** The Vite dev server proxied the real backend on the seeded sandbox database. Each screen was compared with its PNG preview:
- **Dashboard** (light and dark): layout, spacing, type, the attention band, stat cards, the map card, the queue and the consignment rows agree with the mockup. Differences are the deliberate ones in step 2's entry. Not seen in a real browser: the foot of the consignment list (checked in tests) and the map with anything but CARTO's watermarked tiles (see below).
- **Roadmap** (light and dark, on a consignment with two flagged documents): header band, meta row, single tab, category groups and counts, tag and badge placement, and the red flagged rows all agree. Differences: no aside (the Guardian panel is out of scope), so the list sits left of empty space on a wide screen; no Upload button on these rows because the sample importer's flags do not allow it; categories show "Certifications" and "Customs & logistics", which are the invented categories from `db:seed` still in the sandbox (an open item below), not something the app writes.
- **Issue** (light and dark): breadcrumb, title and pill, stepper, Problem card with Expected and Found boxes, Activity, the two side cards and the gold and plain buttons all agree, and computed styles were read back (system-ui at 500 for body text, the serif at 700 for names, 14px captions). Differences are those in step 4's entry: no AI byline or Guardian actor, no comment box, no "Coordinated via exporter" line, and "Mandatory" replaced by who the item is required from.
- **Intake** (light and dark, including the chosen-file state driven through the real file input): step cards, numbered circles, dashed drop zone, the green "ready" tint, two-column fields and the gold button agree. Differences: no AI-extracted pills or IOTA line, countries and the exporter are selects, and "Continue to Compliance Roadmap" is replaced by the submit button.
- **Superadmin console** (light and dark, on two real pending organizations): the top bar with the pending count, the queue with the selected row highlighted, the 560px detail panel and the permission table agree. Differences: the table shows the sandbox's real rules (for an exporter, all "No" for edit, download and approve, which is what the API returns) grouped by role, "n/a" is muted rather than red, and there is no Country field.
- **Not checked in a real browser:** narrow (phone and tablet) widths on every screen, the keyboard focus ring on the hidden file input, real drag and drop, and every confirmation dialog. The dialogs and focus behaviour are covered by tests; the responsive rules are written (media queries at 1100, 1000, 900, 800 and 600px) but not looked at.

**Side effect in the sandbox.** To give the superadmin screen something real to show, two pending organizations were created in the local sandbox database through the app's own `proposeOrganization`: "Sample Applicant Exporter (visual check)" and "Sample Applicant Lab (visual check)". They are still pending and can be approved, rejected or left. No other data was changed, and nothing outside the local sandbox was touched.

**Results.** Backend 331, web 244 in fourteen files. **Total: 575**, confirmed by `node scripts/memory-check.mjs --full`. `vite build` is clean (the map is its own 68 KB gzipped chunk; the main bundle is 107 KB gzipped).

**UI-2 status: complete, with three things that need Thomas** (also in `docs/PROJECT_MEMORY.md`): (1) the map's tile provider now returns a watermark, so the map needs a provider or key decision; (2) the mutation checks for steps 2 and 5 did not finish (the system stopped them for low memory) and should be re-run; (3) the earlier open decisions (invented `db:seed` categories; `POST /purchase-orders` returning 500 for a malformed exporter id). Also unchanged and still true: the sidebar's Parties, Issues, Documents and Settings are disabled placeholders, and nothing about vessel tracking (UI-3) or Stripe (Prompt 4) has been started.

---

# UI-3: vessel tracking

Source: `docs/veripura-cli-ui-prompts.md`, Prompt UI-3 (five numbered steps), as rewritten around VesselAPI's free plan of 150 calls a month. Ground rules that apply to every step: the browser never calls a provider; opening the dashboard, reloading it or refetching the API never causes a provider call; a position that is not real is never drawn; no key in the repo, the build log, fixtures, logs or responses; no network in any test.

## 2026-09-19, UI-3 step 1: schema and vessel identifiers

**Built** (backend only; the web app is step 4)
- **Migration `0002_vessel_tracking_identifiers_and_positions`**, generated by drizzle-kit and applied to the local sandbox database: three nullable columns on `consignments` (`vessel_imo`, `vessel_mmsi`, `vessel_name`), so every existing row stays valid, with CHECK constraints that the IMO is seven digits and the MMSI nine (a second line of defence; the check digit itself is checked in code); and the new `vessel_positions` table exactly as specified (id, IMO and MMSI, lat, lng, speed, heading, nav status, position time, received time, source), with indexes on (MMSI, position time descending) and (IMO, position time descending).
- **`vessel_positions` also carries integrity rules the prompt did not list:** a row must have an IMO or an MMSI, lat and lng are range-checked, and a unique index per vessel and reported time (on the MMSI when there is one, on the IMO otherwise) so re-fetching the same position cannot store it twice. Step 2's ingestion relies on that for its deduplication. Pruning to 72 hours is step 2, with the rest of the ingestion service.
- **`src/tracking/identifiers.ts`:** the IMO check digit (first six digits times 7, 6, 5, 4, 3, 2, summed, last digit of the sum equals the seventh), MMSI as exactly nine digits, and a vessel name of at most 100 characters. Leading zeros survive (an MMSI can start with 0), so a number sent as JSON is refused with "must be sent as text" instead of being quietly turned into a shorter string. Text is trimmed; blank or null means "no value". A request with several problems gets one 422 listing all of them.
- **New error class and status:** these are answered **422** (`{ "error": "unprocessable", "message": ... }`). The app's existing validation errors are 400 and are unchanged; the prompt asks specifically for 422 for vessel identifiers.
- **`POST /consignments`** accepts `vesselImo`, `vesselMmsi`, `vesselName` (all optional). They are checked before the purchase order file is stored, so a bad one creates nothing. When any is given, the "consignment.po_submitted" audit row records them; when none is, that row is unchanged.
- **`PATCH /consignments/:id/vessel`** (JSON) sets or clears them. Send any of the three; an absent field is left alone, null or blank clears it. It answers with the consignment's detail. It writes "consignment.vessel_updated" with the old and new values of all three fields, in the same transaction as the change, on a row locked for the update so two simultaneous edits cannot both record the same "old" value. A request that changes nothing answers 200 with no audit row (nothing happened, so there is nothing to record). An empty body is a 422.
- **`GET /consignments` and `GET /consignments/:id`** now return `vesselImo`, `vesselMmsi`, `vesselName` (null when unset).

**Decision for Thomas: who may set the vessel.** The prompt says "only an org that may edit the consignment", using the permission engine. That engine grants rights per document type, not per consignment, so it has no "edit the consignment" grant to consult. I used the same rule as submitting the purchase order: **any active user of the importing organization, and superadmin.** The exporter is a party and can see the consignment, so it gets 403 rather than 404. A user who is not a party gets exactly the 404 a missing consignment gets, and permission is checked before the body is read, so a stranger cannot learn anything from a 422. If the exporter or logistics should be able to enter the vessel (the exporter often books the ship), that is a one-line change in `src/services/vessel.ts`; it is a real question about who owns the tracking data, so I did not widen it unasked.

**Verification**
- 33 new backend tests (`tests/vesselIdentifiers.test.ts`): the check digit (every wrong digit, each weight, only the last digit of the sum counts), MMSI and length rules including leading zeros, parsing (trim, clear, absent, non-text, several problems in one message, the name limit at the boundary), create with and without the fields and the 422 for each kind of bad value creating nothing, PATCH (set, partial change with the audit showing the value before, clear, no-op writes no audit, bad value and empty body change nothing and audit nothing, both parties and superadmin, exporter 403, stranger and malformed id the same 404, permission before validation), the read models, and the database's own constraints and deduplication. Two existing exact-shape tests (`consignmentDetail`, `webFields`) were updated on purpose to include the three new keys.
- **Mutation check, complete:** 28 mutants, one at a time, in the foreground, all caught (the weights, the modulus, the seventh digit ignored, IMO and MMSI length and digit rules, the name limit off by one, blank not clearing, no trimming, non-text accepted, only the first problem reported, absent fields cleared, null not clearing, the importer-only rule removed or widened to the exporter, the non-party leak, no audit, a no-op audited, old and new swapped, an empty body accepted, untouched fields wiped, create ignoring or not auditing the vessel, each read model missing a field, 422 becoming 400, a non-object body accepted).
- Backend 364, web 244. **Total: 608**, confirmed by `node scripts/memory-check.mjs --full`. `tsc --noEmit` clean.

**Not in this step:** the provider interface and budget (step 2), the position endpoints (step 3), and the web app's fields and validation messages (step 4; a parity test will keep the web copy of the identifier rules identical to this one).

## 2026-09-19, UI-3 step 2: position provider, call budget and refresh job

**What was relied on from VesselAPI's published documentation** (read on 2026-09-19 at vesselapi.com: `/docs`, `/docs/vessels`, `/api-reference`, `/pricing`; nothing here was guessed, and where the pages did not say, that is stated)
- **Base URL and authentication:** `https://api.vesselapi.com/v1`; `Authorization: Bearer <key>` in a header. The key is never put in a URL.
- **How a vessel is identified:** by **MMSI or IMO**, chosen per request with the required parameter `filter.idType` (`mmsi` or `imo`). One request carries one kind.
- **Endpoint used (default): batch, `GET /vessels/positions`.** Parameters: `filter.ids` (comma-separated, or repeated), `filter.idType`, optional `time.from` and `time.to` (RFC 3339; the API defaults to the past 2 hours), `pagination.limit` (1 to 50, default 20), `pagination.nextToken`. **One call can return several vessels: yes.** This is the endpoint that returns the most vessels per call, so it is the default. It returns `vesselPositions[]` with `mmsi`, `imo`, `vessel_name`, `latitude`, `longitude`, `timestamp` (UTC ISO 8601), `processed_timestamp`, `cog`, `sog` (knots), `heading`, `nav_status` (a numeric code), `suspected_glitch`, and a GeoJSON `location`. Null is used for unavailable speed, course, heading and status.
- **Fallback (opt-in, `VESSELAPI_LOOKUP=single`): `GET /vessel/{id}/position?filter.idType=...`,** one vessel per call, described as the most recent AIS position, looking back at most 80 hours and answering 404 if older. The `/api-reference` page lists this path differently (`/v1/vessel/{id}`); the `/docs/vessels` page states `/vessel/{id}/position` explicitly, so that is what is used. It is not the default, so this only matters if someone opts in.
- **Satellite:** `filter.sat` exists and spends separate one-time credit packs. It is **never sent**.
- **Errors listed:** 400, 401, 402 (satellite credits), 403, 404, 429, 500. A rate limit exists (rolling 5-minute windows: 500 per IP, 3,000 per key, 300 per key for location endpoints); those are far above what this stage will ever send.
- **The free plan:** "150 API calls / month". Paid plans on the same page: Basic 1,500 a month, Starter 15,000, Growth 30,000, Pro "millions". Terrestrial AIS is included on every plan.
- **What counts as a billable call:** the documentation says **"only successful (2xx) responses count against the quota"**. The prompt asked for the stricter assumption that failed calls may count, and the budget follows the prompt: **every call is counted, failed ones included**. Being stricter costs nothing; being looser could cost the last calls of the month.

**What the documentation does not say (and how the code copes)**
- **Whether the batch endpoint returns only the latest position per vessel, or every position inside the time window.** The page says only "retrieve position data for multiple vessels in a single request". So the parser accepts several rows per vessel and stores them all (the ingestion deduplicates), only the first page is ever read (a second page is a second billable call), and there is one fallback (`single`) if a live test shows the batch answer crowds vessels out of its 50 rows.
- **A maximum number of ids per batch call, and whether one batch call counts as one call or as one per vessel.** Not stated. The default batch size is 20 (configurable up to 50). **Before relying on it, make one manual refresh and check the count on VesselAPI's own dashboard against `GET /admin/tracking/budget` (step 3).** If a batch of N counts as N, set `VESSELAPI_BATCH_SIZE=1`.
- **Commercial-use terms.** No terms of service or acceptable-use page was found on the pages read; the main page links only a privacy policy. The free plan is treated as **for evaluation only**, and its commercial-use terms are **not confirmed**. That is why a live provider will not start unless `AIS_LIVE_ALLOWED=true`: so nobody switches live data on for paying customers by accident. What must be decided before live data is turned on for customers is in `docs/tracking.md`.

**Built** (`src/tracking/`, all backend)
- **`provider.ts`:** the `VesselPositionProvider` interface (`planRequests` splits identifiers into requests, each exactly one billable call; `fetchPositions` makes one), the normalised position, and validation: latitude within -90 and 90, longitude within -180 and 180, a readable time not more than a minute in the future, a usable identifier. A bad position is dropped and the drop is logged as a count, never as its contents. An optional field that is out of range (a negative speed, a heading of 511 which AIS uses for "not available") is left out and the position kept, since the position itself is still true.
- **`sampleProvider.ts`:** the default. Deterministic great-circle positions for any identifier (the same vessel at the same moment always gets the same place), tagged source `sample`, moving along one of three routes as time passes; one vessel in four reports a position hours old, so "stale" can be seen. It calls nothing and needs no key.
- **`vesselApiProvider.ts`:** the live provider, as described above. No retries and no sleeping: a failed request is one recorded call and the next scheduled run tries again. 401, 403 and 5xx are failures; a 404 from the single-vessel endpoint means "no position", still counted; a 429 sets a back-off. Error messages are built from the status only (never from a response body or a URL), and pass through a redaction of the key as well.
- **`budget.ts` (the call budget):** the `provider_calls` table (provider, called_at, purpose, status, vessels_requested, and what a 429 asked to wait). Before **every** call the ledger counts this calendar month's calls (UTC) from that table and refuses unless `used + 1 + reserve <= budget`, so with 150 and 15 at most 135 automatic calls a month. It takes a database lock so callers at the same moment cannot both take the last call, and it writes the row **before** the request is sent (as `started`) and completes it afterwards, so a crash cannot lose a call that may already have been counted. It warns in the log from 80 percent. The check lives **inside the provider**, so no code path can skip it. A 429 sets a back-off (15 minutes, doubling for each 429 in a row, capped at 6 hours, or the `Retry-After` if longer), read from the same table, so it survives a restart.
- **`refresh.ts` (the refresh job), the only code that calls a provider:** picks vessels on live consignments (not completed or cancelled) that have an MMSI or IMO and whose latest stored position is older than the per-vessel minimum age (default 12 hours) or missing; orders them open issues first, then no position, then oldest position; asks in as few calls as the provider allows; stores the result. **It does nothing, and writes nothing, when no consignment has a vessel identifier.** A scheduled run on a live provider may spend at most **today's allowance: (budget, less the reserve, less what was used before today) divided by the days left in the month counting today, less what today already used**; over a month that spreads the plan rather than front-loading it. The ledger still enforces the budget on each call. A scheduled run is never sooner than the minimum interval after the last one, **even across restarts** (the last run is read from the database), so restarting the server cannot spend calls. Every run that attempted anything is recorded in `tracking_runs`.
- **`ingest.ts`:** stores validated positions, deduplicating on (vessel, reported time) through the unique indexes from step 1; `pruneOldPositions` deletes anything reported more than 72 hours ago, and every scheduled tick does it before refreshing.
- **`config.ts` and `runtime.ts`:** all settings from the environment (`AIS_PROVIDER`, `AIS_LIVE_ALLOWED`, `VESSELAPI_KEY`, `VESSELAPI_MONTHLY_BUDGET`, `VESSELAPI_RESERVE`, `VESSELAPI_LOOKUP`, `VESSELAPI_BATCH_SIZE`, `VESSELAPI_WINDOW_HOURS`, `VESSELAPI_BASE_URL`, `TRACKING_REFRESH_INTERVAL_MINUTES`, `TRACKING_REFRESH_MIN_INTERVAL_MINUTES`, `TRACKING_MIN_POSITION_AGE_MINUTES`, `TRACKING_RECENT_MAX_AGE_SECONDS`, `TRACKING_REFRESH_ENABLED`), each with a safe default and validated on load. **`AIS_LIVE_ALLOWED` must be exactly `true` for a live provider to start;** without it (or without a key) the server refuses to start. The refresh interval can never be set below its floor. **`buildApp` uses sample positions whatever the environment says, unless it is handed a runtime;** only `src/server.ts` builds one from the environment, so no test can reach a live provider by accident.
- **`POST /admin/positions/refresh`:** superadmin only; runs one refresh now and answers with counts. It still obeys the budget, and still skips vessels whose position is recent, so pressing it twice spends nothing the second time.
- **`.env.example`** now lists every variable with the key blank, and `.env` is git-ignored (checked). The seed's three live sample consignments now carry made-up vessels whose numbers cannot belong to a real ship (IMO numbers in the unissued 1000000 range, MMSI prefix 999), so switching to a live provider later could never track a real vessel on sample data.

**Decisions for Thomas**
- **The reserve.** The prompt says the reserve is "kept back for manual use", and that a manual refresh "still obeys the budget". I read those together: an automatic call must leave the reserve untouched, a **manual** call may use it, and nothing may ever pass the monthly budget. The ledger enforces the strict rule (`used + 1 + reserve <= budget`) unless the caller says it is a manual call. If you would rather the reserve be off limits to everyone, it is one argument in `src/tracking/refresh.ts`.
- **A manual refresh skips the daily allowance** (it is a person's decision, and rare) but not the per-vessel minimum age.
- **Two ways to ask VesselAPI, defaulting to the one the prompt prefers.** `VESSELAPI_LOOKUP=single` exists only because the documentation does not settle what the batch endpoint returns.

**Verification**
- **111 new backend tests** in five files: validation (edges of every range, the one-minute future limit exactly, unreadable times, identifiers, optional fields, the dropped count logged and never the contents); the sample provider (deterministic, valid, moving, some stale); the ledger (the budget at its boundary, only this provider's calls, the row written first, failed and unfinished calls counted, a new calendar month in UTC, the reserve for manual calls only, no overspending under 30 simultaneous callers, the 80 percent warning, back-off length, doubling and cap, `Retry-After`, surviving a restart, ending on success); the VesselAPI provider against a **fake network** (the exact URL, parameters, header and no satellite flag; field mapping; glitches and bad rows dropped; one page only; 401, 403 and 5xx as single un-retried failures; timeout; 404; a 429 setting a back-off so the next attempt sends nothing; the key in no message, log, ledger row or URL); and the refresh (nothing to do with no vessels, ordering, the daily allowance including a **30-day simulation** that never exceeds a day's allowance or the budget less the reserve, no burst after a restart, stopping at the first refused call or 429, a failing provider never breaking the tick, deduplication, 72-hour pruning at the boundary, the admin endpoint's 401, 403, budget and repeat behaviour, and the scheduler). **No test touches the network.**
- **Mutation check, complete: 73 mutants,** in the foreground or a monitored background run. Sixty-eight were caught first time. Five survived and were closed: a latent inconsistency in the provider (the chosen identifier type and its value came from two different rules, so a vessel with both numbers could be asked about with the wrong one; now one function decides both, and a test pins it); a shared vessel's open-issue flag depending on row order (the query is now ordered, and the test no longer depends on how rows sit on disk); an ordering test where ids hid the "no position first" rule; an allowance case where rounding hid a double count of today's calls; and a scheduler test that only proved `stop()` did not throw (it now checks that a disabled scheduler runs nothing and an enabled one runs a tick). Not reachable by any test: the redaction on the provider's generic catch-all (nothing on that path can contain the key, so it is defence in depth) and the "more pages" warning text.
- Backend 475, web 244. **Total: 719** (see the count in `docs/PROJECT_MEMORY.md`, confirmed by `node scripts/memory-check.mjs --full`). `tsc --noEmit` clean. Migration `0003_provider_calls_and_tracking_runs` applied to the sandbox.

**Not in this step:** the position endpoints and `GET /admin/tracking/budget` (step 3), the map wiring (step 4), and `docs/tracking.md` (step 5). No live call has been made: there is no key in this sandbox, and the first live use is a decision for Thomas.

## 2026-09-19, UI-3 step 3: position endpoints

**Built** (`src/services/positions.ts`, `src/http/tracking.ts`; backend only)
- **`GET /positions`:** every consignment the user can see, in the same order as `GET /consignments` (newest first), each as `{ consignmentId, freshness, reason, lat, lng, speedKnots, headingDeg, positionTime, ageSeconds, isSample, trail }`. Answers `{ "positions": [...] }`.
- **`GET /consignments/:id/position`:** the same item for one consignment. A consignment that does not exist, is malformed, or belongs to someone else gets one and the same 404.
- **Freshness** is `recent` if the position is at most the threshold old (exactly that old still counts), `stale` if older, `unavailable` if there is no position. **The threshold is configuration** (`TRACKING_RECENT_MAX_AGE_SECONDS`, default 7200), not a literal in the code; a test proves a shorter configured value changes the answer. `ageSeconds` is whole seconds rounded down and never negative (a position a moment ahead of our clock counts as brand new).
- **A consignment with no vessel identifier is never omitted.** It comes back `unavailable` with `reason: "no_vessel_identifier"`. A vessel with an identifier but no stored position comes back `unavailable` with `reason: "no_position_received"`, so the map can say which, as the prompt asks for the info line. Every position field is null in both cases and `isSample` is false.
- **`isSample`** is true only when the position's source is `sample`. A real position is never flagged sample, and a sample one is never unflagged.
- **The trail** is the last 24 hours of that vessel's positions, oldest first, thinned to at most 100 points by taking evenly spaced ones and always keeping the first and the last. A moment stored under both the MMSI and the IMO is drawn once. It is included by default and left out with `?trail=false` (any other value is a 422). An unavailable consignment gets an empty trail when one is asked for, and none when it is not.
- **Which positions belong to a consignment:** those stored under its MMSI or its IMO, so a vessel entered by IMO alone finds a position the provider stored under that IMO.
- **`GET /admin/tracking/budget`** (superadmin only): the provider in use and whether it is live, the month, calls used, the budget, the reserve, calls remaining (reserve included) and remaining for automatic runs, the percent used, a running 429 back-off if any, the date of the last refresh, and that refresh's outcome. It reports counts and dates only: no key, no request, no provider data. For the sample provider it reports zero calls, because none are made.
- **Visibility is exactly `GET /consignments`'s:** a party (importer or exporter) sees its own consignments' positions, superadmin sees all, and a stranger's list simply does not contain them. The service applies this itself, so it does not depend on the route. Two importers whose consignments happen to be on the same ship each see only their own.

**Reading never calls a provider, and that is enforced, not assumed.** The position service imports no provider code, and the read handlers never touch the provider. A test makes every read the dashboard makes (`/me`, `/consignments`, the detail, the checklist, `/positions` with and without the trail, the single position, the action queue, party workload, and the admin budget), three rounds over, against a **live provider wired to a fake network**, and asserts no request was made and no `provider_calls` or `tracking_runs` row was written. It also checks a vessel that has never been refreshed shows "no position received" without triggering anything.

**No key in any response.** A test wires a live runtime whose fake provider answers 401 with the key echoed in the body, runs a manual refresh, and then reads the budget, position and admin responses (also as the wrong users) plus every log line, ledger row and run row: the key appears nowhere, and neither does "Bearer". The consignment routes, which now carry vessel fields, are checked the same way.

**Verification**
- 46 new backend tests (`tests/positions.test.ts`): freshness at both boundaries and with another threshold; age rounding and clamping; trail thinning (short, long, first and last kept, ordered, no repeats, evenly spaced, another maximum); recent, stale, sample, no identifier, no position; latest position of several; matching by IMO, by MMSI, by both, not mixing two vessels; ordering; an empty list; a vessel cleared from the consignment; the trail (24-hour window, oldest first, deduplicated, thinned to 100 with the newest and oldest kept, left out on request even when unavailable, 422); visibility for each party, a stranger (no coordinates and no consignment id anywhere in the response), two importers on one ship, superadmin, the identical 404, 401, and the service itself; zero provider calls; and the budget endpoint's 401, 403, contents before anything has happened, this month only with failed calls counted, the configured budget and reserve, the last refresh after a real one, a 429 back-off, and the sample provider.
- **Mutation check, complete: 32 mutants,** all caught in the end. Twenty-nine were caught first time. Three survived and were closed with tests: age rounded instead of rounded down (my test used .9 seconds, where both agree; it now also uses .4), and a `?trail=false` request still returning a trail for a consignment with no position (two mutants, one test). The mutants covered the threshold comparison and null handling, age arithmetic, thinning, the trail window and order and de-duplication, MMSI and IMO matching, latest versus oldest position, the sample flag, both reasons, visibility (everyone sees all, exporter cannot see, the single endpoint leaking), ordering, the trail parameter, and each field of the budget response and its permission check.
- Backend 521, web 244. **Total: 765**, confirmed by `node scripts/memory-check.mjs --full`. `tsc --noEmit` clean.

**Not in this step:** the web app (step 4) and `docs/tracking.md` (step 5). There is no pagination on `GET /positions`, the same as `GET /consignments`; each consignment costs two small queries (its latest position and its trail), which is fine for one organization's few consignments and is a known limit if that changes.

## 2026-09-19, UI-3 step 4: frontend wiring

**Built** (`web/`)
- **The dashboard map is fed by `GET /positions`,** not by `mapSample.ts`. `usePositions` reads it through TanStack Query and **refetches every 60 seconds, only while the tab is showing** (`refetchIntervalInBackground` is off, so a forgotten tab does no work). It reads only our own database: the server has no path from that endpoint to a provider (step 3), and the browser never calls a provider or any host but `/api` (a grep-style test fails if any file under `web/src` names one).
- **One entry per live consignment** (`screens/dashboard/mapModel.ts`), in the list's order: a consignment with a position is drawn; one without is still a chip whose info line says why. The colour comes from the consignment (red with an open issue, green "cleared" once every required document is verified, otherwise "on track"), not from how fresh the position is.
- **Markers by freshness.** "recent" is a normal marker. "stale" is the hollow dashed marker. "unavailable" has **no marker**, and the info line says **"No vessel identifier on this consignment"** or **"No position received yet"**. The info line shows the last position's age, speed and heading when known, worded "Last known position" for a stale one so it is never read as current, and "(sample)" for a sample one. "Open roadmap" is back, since these are real consignments now.
- **The flag.** "Sample positions" whenever any drawn position is sample data, even beside real ones. Otherwise "Live AIS", but **only if a drawn position is recent**: if every drawn position is stale it says "Last known positions", and with nothing drawn "No vessel positions". "Live" is never shown for stale data (a test scans the whole card for the word).
- **No planned route.** The backend has no port or route data, so nothing draws an arc from origin to destination. The vessel's own reported **trail** (the last 24 hours, from step 3) is drawn as a thin line in the marker's colour, thicker for the selected consignment. A trail of one point draws nothing.
- **The map redraws on refresh without being rebuilt.** Markers and trails are redrawn when the positions change (a new marker, a moved one, one that disappears), the tile layer is not recreated, and the user's view is not moved after the map first frames the vessels. A refetch that returns identical data changes nothing, because the dashboard only rebuilds the vessel list when the data actually changes.
- **A bug fixed on the way:** the trail's width was set in CSS as well as by Leaflet, and CSS wins, so the "thicker when selected" emphasis from step 2 never showed in a real browser (the tests read the attribute, which was right). The CSS width is gone; Leaflet's own weight controls it.
- **`mapSample.ts` is now only a fixture** (with a header saying so), rewritten to the new shape (recent, stale, and no-identifier vessels). The dashboard does not import it, and a hygiene test fails if any shipped file does.
- **Roadmap header** shows the vessel's name, IMO and MMSI, each only when present, with no placeholder when there is none.
- **Intake** has three optional fields (vessel name, IMO, MMSI), checked with **the same rules and the same words as the API**, sent only when filled, trimmed. They come from `web/src/vessel.ts`, a copy of the API's rules, and **`tests/vesselIdentifierParity.test.ts` (in the backend suite) runs both over the same inputs, every possible check digit and a range of 3,000 IMO numbers included, so the two cannot drift apart.** If the form lets something through that the API refuses, the API's own 422 message is shown.
- `useSubmitConsignment` now also marks the positions stale, since a new consignment adds one.

**Verification**
- **49 new web tests and 6 new backend tests** (web 293 in sixteen files, backend 527). The map tests were rewritten around the API: the credit, every flag case, the tile swap with Leaflet's tile layer replaced by a recorder (no network), each marker state, colours, finished consignments left out, no default Leaflet image, trails (drawn, none for one point, no arc, colour, thin, emphasis on the selected one, a marker click selecting), the info line in every case, the default selection, the empty and loading and error states with retry, the 60-second refetch options, and the refresh behaviour (a marker moving, appearing and disappearing without rebuilding the map or requesting new tiles). Separate tests cover the model (state, live filter, no invented position, trail, wording, the flag) and the web copy of the identifier rules.
- **Mutation check, complete: 46 mutants,** all caught in the end. Forty-four were caught first time. Two survived and were closed: the emphasis test counted one thick and one thin trail but not which was thick (it now uses a red and a green trail and checks each), and nothing clicked a marker on the map (a test now does). The mutants covered the identifier rules, the model, the wording and rounding of the info line, the flag rules, the marker classes, the trail rules, selection, the default selection, the roadmap link, the refetch interval and background setting, the map's error state, and the intake and roadmap fields.
- Two existing dashboard tests needed their queries scoped to the consignment list, because the map's info line now repeats the selected consignment's title. That is the intended change, not a regression.
- Backend 527, web 293. **Total: 820**, confirmed by `node scripts/memory-check.mjs --full`. `vite build` is clean.
- **Checked against the real stack, over HTTP, not in a browser.** With the real backend, the real database and the sample provider: three consignments were given made-up vessels through `PATCH /consignments/:id/vessel`, a manual refresh stored three positions, `GET /admin/tracking/budget` reported them (zero calls used, since the sample provider calls nothing), and `GET /positions` through the Vite proxy returned two recent sample positions, one stale (about 5 hours old), trails of 10 and 11 points, and the two finished consignments as unavailable with their reason. **A real-browser look at the map was not possible this session:** the browser extension refused the tab ("unparseable URL") on several fresh attempts, so I stopped rather than keep retrying. What is therefore unchecked visually is the look of the trail lines, the hollow marker and the new flag colours; the step 2 visual check of the map (markers, labels, credit, both themes) still stands.
