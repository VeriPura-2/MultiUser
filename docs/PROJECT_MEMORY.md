...
Tests passing: 487
Stages complete: 3 of 3 backend prompts, and UI-1 (API surface for the UI). UI-2 (the web app) in progress, step 3 of 7 done. UI-3 (vessel tracking) and Prompt 4 (Stripe) not started.
...
1. **UI-2** (the React web app), from `docs/veripura-cli-ui-prompts.md`, steps 4 to 7 remaining (issue, intake, superadmin approval, final tests and the visual comparison against the PNG previews). The roadmap and the dashboard link to `/issues/:id`, which is not routed until step 4. Then **UI-3** (vessel tracking, from the same file). The mockups are in `docs/ui-mockups/v2-revised/` (untracked, Thomas's design inputs).
