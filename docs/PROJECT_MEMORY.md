...
Tests passing: 520
Stages complete: 3 of 3 backend prompts, and UI-1 (API surface for the UI). UI-2 (the web app) in progress, step 4 of 7 done. UI-3 (vessel tracking) and Prompt 4 (Stripe) not started.
...
1. **UI-2** (the React web app), from `docs/veripura-cli-ui-prompts.md`, steps 5 to 7 remaining (intake, superadmin approval, final tests and the visual comparison against the PNG previews). The dashboard's "+ New Consignment" arrives with the intake screen in step 5. Then **UI-3** (vessel tracking, from the same file). The mockups are in `docs/ui-mockups/v2-revised/` (untracked, Thomas's design inputs).
